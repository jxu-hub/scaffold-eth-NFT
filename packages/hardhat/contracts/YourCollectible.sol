	...
	ReentrancyGuard
{
	...
	struct BlindBox {
		uint256 price; // 盲盒价格
		uint256[] nftPool; // 盲盒内的NFT Token ID池
		bool isActive; // 是否激活盲盒
	}
	mapping(uint256 => BlindBox) public blindBoxes; // 存储不同类型盲盒的信息
	uint256 public blindBoxCounter; // 盲盒类型计数器
	...
	// 事件
	event BlindBoxCreated(uint256 blindBoxId, uint256 price);
	 ...
	 * @param price 盲盒价格
	 */
	function createBlindBox(uint256 price) external onlyOwner {
		blindBoxCounter++;
		blindBoxes[blindBoxCounter] = BlindBox({
			price: price,
			nftPool: new uint256[](0),
			isActive: true
		});

		emit BlindBoxCreated(blindBoxCounter, price);
	}

	uint256 public tempTokenId;
	...
	function purchaseBlindBox(
		...
		require(box.isActive, "Blind box is not active");
		require(msg.value >= box.price, "Insufficient payment");
		require(box.nftPool.length > 0, "No NFTs left in the blind box");
		...
		uint256 tokenId = box.nftPool[randomIndex];

		// 将选中的NFT转移给购买者
		_transfer(address(this), msg.sender, tokenId);

		// 从盲盒池中移除已分配的NFT
		box.nftPool[randomIndex] = box.nftPool[box.nftPool.length - 1];
		box.nftPool.pop();
		tempTokenId = tokenId;
		emit BlindBoxPurchased(msg.sender, blindBoxId, tokenId);
		return tokenId;
	}
	...
	/**
	 * @dev 批量铸造NFT并将它们分配到盲盒
	 * @param blindBoxId 盲盒ID
	 * @param quantity 铸造数量
	 * @param uris NFT的元数据URI列表
	 */
	function batchMintToBlindBox(
		uint256 blindBoxId,
		uint256 quantity,
		string memory categories,
		string[] memory uris
	) external onlyOwner {
		require(blindBoxes[blindBoxId].isActive, "Blind box is not active");
		require(quantity == uris.length, "Quantity must match URIs length");
		require(quantity > 0, "Must mint at least one NFT");

		for (uint256 i = 0; i < quantity; i++) {
			string memory rarity = _calculateRarity(uris[i]);
			tokenIdCounter.increment();
			uint256 tokenId = tokenIdCounter.current();
			_mint(address(this), tokenId); // 铸造NFT到合约地址
			_setTokenURI(tokenId, uris[i]); // 设置NFT的URI
			...
			// 将Token ID加入盲盒NFT池
			blindBoxes[blindBoxId].nftPool.push(tokenId);
		}
	}
	 ...
	 * @param blindBoxId 盲盒ID
	 */
	function getBlindBoxRemaining(
		uint256 blindBoxId
	) external view returns (uint256) {
		return blindBoxes[blindBoxId].nftPool.length;
	}

	/**
	 * @dev 获取所有盲盒的详细信息
	 * @return 一个包含所有盲盒详细信息的数组
	 */
	function getAllBlindBoxes() external view returns (BlindBox[] memory) {
		BlindBox[] memory allBlindBoxes = new BlindBox[](blindBoxCounter);

		for (uint256 i = 1; i <= blindBoxCounter; i++) {
			allBlindBoxes[i - 1] = blindBoxes[i];
		}

		return allBlindBoxes;
	}
	 ...
	 * @param blindBoxId 盲盒ID
	 */
	function deactivateBlindBox(uint256 blindBoxId) external onlyOwner {
		blindBoxes[blindBoxId].isActive = false;
	}
